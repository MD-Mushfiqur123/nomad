...
func (d *NvidiaDevice) SetConfig(cfg *base.Config) error {
	...
	if config.MpsConfig != nil {
		...
		d.logger.Info("not nil", "config", fmt.Sprintf("%+v", config.MpsConfig))
		...
		d.MpsConfig.MpsSockFile = checkAndSetDefault(config.MpsConfig.MpsSockFile, DefaultMpsSockFileAddr)
